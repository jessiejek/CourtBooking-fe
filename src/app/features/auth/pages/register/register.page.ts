...
export class RegisterPage implements AfterViewInit {
  ...
  private async initializeGoogleButton(): Promise<void> {
    ...
    try {
      ...
      this.googleSdkError = error instanceof Error ? error.message : 'Google sign-up failed to initialize.';
    }
  }
  ...
  private async handleGoogleCredential(response: GoogleCredentialResponse): Promise<void> {
    ...
    this.auth
      ...
      .subscribe({
        ...
        error: (err) => (this.error = err.message || 'Google sign-up failed.'),
      });
  }

  async onGoogleFallbackClick(): Promise<void> {
    if (!this.canUseGoogleLogin) {
      this.error = 'Google sign-up is not configured yet.';
      return;
    }
            ...
            if (response.authResponse?.accessToken) {
              this.auth
                ...
                .subscribe({
                  next: () => this.router.navigateByUrl(this.returnUrl || this.getRedirectUrl()),
                  error: (err) => {
                    this.error = err.message || 'Facebook sign-up failed.';
                    this.facebookLoading = false;
                  },
                });
            } else {
